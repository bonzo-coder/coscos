...
const primaryPhone = "+48227507320";
const secondaryPhone = "+48600392004";
const contactRecipient = "info@easylink.com.pl";
...
const contactCopy = {
  pl: {
    ...
    form: {
      ...
      error: "Nie udało się wysłać formularza. Spróbuj ponownie za chwilę albo napisz na info@easylink.com.pl.",
      missingConfig: "Formularz nie jest jeszcze skonfigurowany do wysyłki. Uzupełnij ustawienia EmailJS.",
    },
  },
  en: {
    ...
    form: {
      ...
      error: "We could not send the form. Please try again in a moment or email info@easylink.com.pl.",
      missingConfig: "The form is not configured for sending yet. Please complete the EmailJS settings.",
    },
  },
};
...
function ContactPage() {
  ...
  const handleSubmit = async (evt) => {
    ...
    if (!emailJsServiceId || !emailJsTemplateId || !emailJsPublicKey) {
      setSubmitError(copy.form.missingConfig);
      return;
    }
    ...
    try {
      await emailjs.send(
        ...
        emailJsTemplateId,
        {
          ...
          platform_length: formValues.platformLength,
          platform_width: formValues.platformWidth,
      ...
      setFormValues(formFields.reduce((acc, field) => ({ ...acc, [field.name]: "" }), {}));
      setFormErrors({});
      setSubmitted(true);
    } catch {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const getParallaxRange = () => {
        ...
        const fallbackDistance = galleryNode.clientHeight * 0.6;
        const baseDistance = overflow > 0 ? overflow : fallbackDistance;
        ...
        const startOffset = Math.min(Math.max(galleryNode.clientHeight * 0.08, 40), 90);

        return {
          startY: startOffset-130,
          endY: -Math.max(baseDistance - startOffset, galleryNode.clientHeight * 0.5),
      ...
      gsap.fromTo(
        track,
        { y: () => getParallaxRange().startY },
        {
          y: () => getParallaxRange().endY,
          ease: "none",
          scrollTrigger: {
            trigger: heroNode,
            start: () => getParallaxRange().triggerStart,
            end: "bottom top",
            scrub: true,
        ...
        <div className="contact-form__panel">
          ...
          <form className="contact-form__grid" onSubmit={handleSubmit} noValidate>
            ...
            <div className="contact-form__actions">
              ...
              {submitError && <p className="contact-form__error contact-form__error--submit">{submitError}</p>}
            </div>
          </form>
        </div>
      </section>
      ...
      <section className="contact-details">
        <div className="contact-details__panel">
          <div className="contact-details__info">
            ...
            <address>
              <p>
                <a href={mapsUrl} target="_blank" rel="noreferrer">ul. Nefrytowa 12</a>
              </p>
              <p>
                <a href={mapsUrl} target="_blank" rel="noreferrer">05-500 Piaseczno</a>
              </p>
              <p>NIP: 716-104-63-01</p>
              <p>tel. <a href={`tel:${primaryPhone}`}>+48 22 750 73 20</a></p>
              <p>tel. <a href={`tel:${secondaryPhone}`}>+48 600 392 004</a></p>
